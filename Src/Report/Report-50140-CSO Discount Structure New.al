report 50140 "CSO Discount Structure New"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/CSODiscountStructureNew.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                //dataitem(DataItem1000000002; 13795)
                // {
                /* DataItemLink = "Document No." = FIELD("Document No."),
                               "Item No." = FIELD("No."),
                               "Line No." = FIELD("Line No.");
                DataItemTableView = SORTING("Document No.", "Item No.", "Line No.", "Tax/Charge Type", "Tax/Charge Group")
                ...
                begin
                    ...
                    //>>1
                    ItemRec.GET("Structure Order Line Details"."Item No.");

                    SpclFOC := 0;
                    SpclFOCPerlt := 0;
                    recStrOrdLineDet.RESET;
                    recStrOrdLineDet.SETRANGE(recStrOrdLineDet."Document No.", "Sales Line"."Document No.");
                    recStrOrdLineDet.SETRANGE(recStrOrdLineDet."Line No.", "Sales Line"."Line No.");
                    recStrOrdLineDet.SETRANGE(recStrOrdLineDet."Tax/Charge Group", 'FOC DIS');
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        SpclFOCPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        ...
                        StateDiscPerlt := recStrOrdLineDet.Amount / "Sales Line"."Quantity (Base)";
                    END;
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        SpotPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        AVPSanctionPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        NewPricePerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        SpecDiscPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        FOCperlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        MntDiscPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        AddtlDiscPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    IF recStrOrdLineDet.FINDSET THEN BEGIN
                        VATDiffPerlt := recStrOrdLineDet."Calculation Value" / "Sales Line"."Qty. per Unit of Measure";
                    ...
                    END;
                    *///Code Commented 02Jun2019
                      //<<1
                      //>>02Jun2019
                      //ProdDisc := "Sales Line"."National Discount Amount";
                      // ProdDiscPerlt := "Sales Line"."National Discount Per Ltr";
                      //<<02Jun2019
                      //>>2
                ...
                /*  STR25Apr.SETCURRENTKEY("Document No.", "Item No.", "Line No.", "Tax/Charge Type", "Tax/Charge Group");
...
*/
                ...
                begin
                    ...
                    ////STR25Apr.COPYFILTERS("Structure Order Line Details");//25Apr2017
                ...
                begin
                    //>>1
                    //ListPrice := "Sales Line"."Unit Price Incl. of Tax"/"Sales Line"."Qty. per Unit of Measure";
                    //<<1

                    //>>07Sep2017 ListPrice
                    CLEAR(ListPrice);
                    ListPrice := "Sales Line"."List Price";
                    //>>07Sep2017 ListPrice
                end;
            }
            ...
            begin
                ...
                recStructOrdrlnDetail.SETRANGE(recStructOrdrlnDetail."Document No.", "Sales Header"."No.");
 ...
 */
                ...
                recStructOrdrlnDetail.SETRANGE(recStructOrdrlnDetail."Document No.", "Sales Header"."No.");
                recStructOrdrlnDetail.SETRANGE(recStructOrdrlnDetail."Tax/Charge Type", recStructOrdrlnDetail."Tax/Charge Type"::Charges);
                recStructOrdrlnDetail.SETRANGE(recStructOrdrlnDetail."Tax/Charge Group", 'TRANSSUBS', 'TRANSSUBS2');
                IF recStructOrdrlnDetail.FINDFIRST THEN
                    REPEAT
                ...
                V_FocDis := '';
                /* Rec_SD.RESET;
                //Rec_SD.SETRANGE(Rec_SD.Code, "Sales Header".Structure);
 ...
 */
            ...
            begin
                ...
                *///Commented 21Aug2017
                ...
                ExcelBuf.NewRow;
                ExcelBuf.AddColumn('CSO Discount Structure', FALSE, '', TRUE, FALSE, FALSE, '', 1);//1
                ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', 1);//2
                ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', 1);//3
                ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', 1);//4
                ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', 1);//5
    ...
    var
        ...
        CCFCtext: Text[30];
        CCFC: Decimal;
        //recStructOrdrlnDetail: Record 13795;
    ...
    begin
        //>>25Apr2017
        /* ExcelBuf.CreateBook('', 'Auto Discount Details');
        ExcelBuf.CreateBookAndOpenExcel('', 'Auto Discount Details', '', '', USERID);
        ExcelBuf.GiveUserControl; */

        ExcelBuf.CreateNewBook('Auto Discount Details');
        ExcelBuf.WriteSheet('Auto Discount Details', CompanyName, UserId);
        ExcelBuf.CloseBook();
        ExcelBuf.SetFriendlyFilename(StrSubstNo('Auto Discount Details', CurrentDateTime, UserId));
        ExcelBuf.OpenExcel();
        //<<25Apr2017
    end;
    ...
    begin
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn("Sales Header"."No.", FALSE, '', FALSE, FALSE, FALSE, '', 1);//1
        ExcelBuf.AddColumn("Sales Header"."Posting Date", FALSE, '', FALSE, FALSE, FALSE, '', 2);//2
        ExcelBuf.AddColumn(/*"Structure Order Line Details"."Item No."*/'', FALSE, '', FALSE, FALSE, FALSE, '', 1);//3
        ExcelBuf.AddColumn(ItemRec.Description, FALSE, '', FALSE, FALSE, FALSE, '', 1);//4
        ExcelBuf.AddColumn("Sales Line"."Lot No.", FALSE, '', FALSE, FALSE, FALSE, '', 1);//5
        ExcelBuf.AddColumn("Sales Line"."Qty. per Unit of Measure", FALSE, '', FALSE, FALSE, FALSE, '0.000', 0);//6
        ExcelBuf.AddColumn("Sales Line".Quantity, FALSE, '', FALSE, FALSE, FALSE, '0.000', 0);//7
        TQty += "Sales Line".Quantity;//25Apr2017

        ExcelBuf.AddColumn("Sales Line"."Quantity (Base)", FALSE, '', FALSE, FALSE, FALSE, '0.000', 0);//8
        TQtyBase += "Sales Line"."Quantity (Base)";//25Apr2017

        /*
        ExcelBuf.AddColumn(ListPrice+ProdDiscPerlt,FALSE,'',FALSE,FALSE,FALSE,'#,###0.00',0);//9
        TListPrice += ListPrice+ProdDiscPerlt;//25Apr2017
        
        ExcelBuf.AddColumn("Sales Line"."Quantity (Base)" * (ListPrice+ProdDiscPerlt),FALSE,'',FALSE,FALSE,FALSE,'#,#0.00',0);//10 06Feb2018
        TBaseAmt06 += "Sales Line"."Quantity (Base)" * (ListPrice+ProdDiscPerlt) ;//06Feb2018
        *///Code Commented

        //>>02Jun2019
        ExcelBuf.AddColumn(ListPrice, FALSE, '', FALSE, FALSE, FALSE, '#,###0.00', 0);//9
        TListPrice += ListPrice;

        ExcelBuf.AddColumn("Sales Line"."Quantity (Base)" * (ListPrice), FALSE, '', FALSE, FALSE, FALSE, '#,#0.00', 0);//10
        TBaseAmt06 += "Sales Line"."Quantity (Base)" * (ListPrice);//
                                                                   //<<02Jun2019
        ...
        *///Commented 21Aug2017
        ...
        IF ProdDiscPerlt <> 0 THEN BEGIN
            /*
              ...
              ExcelBuf.AddColumn(ROUND(((ListPrice+ProdDiscPerlt)-(ABS(TotalDiscPerlt+(-ProdDiscPerlt))))
                                        *"Sales Line"."Quantity (Base)",0.01),FALSE,'',FALSE,FALSE,FALSE,'#,###0.00',0);//24

              TotalAmount += (((ListPrice+ProdDiscPerlt)-(ABS(TotalDiscPerlt+(-ProdDiscPerlt))))*"Sales Line"."Quantity (Base)");
            ...
            *///Code Commented
            ...
            ExcelBuf.AddColumn(((ListPrice) - (ABS(TotalDiscPerlt + (-ProdDiscPerlt))))
                                      * "Sales Line"."Quantity (Base)", FALSE, '', FALSE, FALSE, FALSE, '#,###0.00', 0);//24

            TotalAmount += (((ListPrice) - (ABS(TotalDiscPerlt + (-ProdDiscPerlt)))) * "Sales Line"."Quantity (Base)");
        ...
        END ELSE BEGIN
            ...
            ExcelBuf.AddColumn(ROUND((ListPrice + (TotalDiscPerlt)) * "Sales Line"."Quantity (Base)", 0.01), FALSE, '', FALSE, FALSE, FALSE, '#,###0.00', 0);//24

            TotalAmount += (ListPrice + (TotalDiscPerlt)) * "Sales Line"."Quantity (Base)";
    ...
    begin
        ...
        *///Commented 21Aug2017
        ...
        //ExcelBuf.AddColumn("Sales Line".Quantity,FALSE,'',TRUE,FALSE,TRUE,'0.000',0);//7
        TQty := 0;//25Apr2017

        ExcelBuf.AddColumn(TQtyBase, FALSE, '', TRUE, FALSE, TRUE, '0.000', 0);//8
        //ExcelBuf.AddColumn("Sales Line"."Quantity (Base)",FALSE,'',TRUE,FALSE,TRUE,'0.000',0);//8
