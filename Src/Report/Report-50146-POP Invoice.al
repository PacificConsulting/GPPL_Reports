report 50146 "POP Invoice"
{
    // 06Feb2017 : Plant Invoice-IL(#50003)
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/POPInvoice.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(PolicyExpirationDate_CompInfo; 'valid up to ' + FORMAT(CompanyInfo1."Policy Expiration Date"))
            {
            }
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(Integer; Integer)
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(DimValueName; DimensionName)
                        {
                        }
                        column(BaseUnitofMeaseure_Item; recItem."Base Unit of Measure")
                        {
                        }
                        column(LineNo_SalesInvoiceLine; "Sales Invoice Line"."Line No.")
                        {
                        }
                        dataitem(FixLayout; Integer)
                        {
                            column(Number_FixLayout; FixLayout.Number)
                            {
                            }

                            trigger OnPreDataItem()
                            begin
                                FixLayout.SETRANGE(Number, 1, 22 - vCount);//02May2017
                                //FixLayout.SETRANGE(Number,1,26-vCount);
                            end;
                        }
                        ...
                        begin
                            ...
                            */
                            ...
                            AssValue := 0;//"Sales Invoice Line"."MRP Price";
                                          // END ELSE BEGIN
                            MRPPrice := /*"Sales Invoice Line"."MRP Price"*/0 * "Sales Invoice Line"."Qty. per Unit of Measure";
                            ...
                            */
                            ...
                            IF "Inventory Posting Group" = 'MERCH' THEN BEGIN
                                vDimSetENtry.RESET;
                                vDimSetENtry.SETRANGE("Dimension Set ID", "Sales Invoice Line"."Dimension Set ID");//02May2017
                                                                                                                   //vDimSetENtry.SETRANGE("Dimension Set ID","Dimension Set ID");
                                vDimSetENtry.SETRANGE("Dimension Code", 'MERCH');
                                IF vDimSetENtry.FINDFIRST THEN BEGIN
                                    vDimSetENtry.CALCFIELDS("Dimension Value Name");
                                    DimensionName := vDimSetENtry."Dimension Value Name";
                                    //MESSAGE('Dimension Name % 1 && Line No. %2  \ %3',DimensionName,"Line No.",DimensionName2);
                                END;
                                DimensionName2 := DimensionName; //02May2017
                            END;
                            //<<
            ...
            begin
                ...
                */
                ...
                RecLocation2.SETRANGE(RecLocation2.Code, "Sales Invoice Header"."Location Code");
                IF RecLocation2.FINDFIRST THEN BEGIN
                    IF (RecLocation2."State Code" = 'UP') AND ("Sales Invoice Header"."Posting Date" >= 20140111D) THEN BEGIN //011114D
                        /*  IF DetailedTaxEntry."Tax Type" = DetailedTaxEntry."Tax Type"::VAT THEN BEGIN
                             TaxDescription := 'TAX' + FORMAT(DetailedTaxEntry."Tax %") + '%' + FormCode;
                         END ELSE
                             TaxDescription := FORMAT(DetailedTaxEntry."Tax Type") + FORMAT(DetailedTaxEntry."Tax %") + '%' + FormCode;
