report 50157 "Deport Invoice IL"
{
    // 06Feb2017 : Plant Invoice-IL(#50003)
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/DeportInvoiceIL.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(BillToPan; 'PAN No: ' + FORMAT(RecCust."P.A.N. No."))
            {
            }
            ...
            column(AddTaxDes; "AddlTax/CessDesc")
            {
            }
            column(AddTaxAMt; "AddlTax/Cess")
            {
            }
            ...
            column(V_freightTaxable; V_freightTaxable)
            {
            }
            column(V_freightNonTaxable; V_freightNonTaxable)
            {
            }
            column(FreightTax; FreightTax)
            {
            }
            column(FreightNonTax; FreightNonTax)
            {
            }
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(Integer; Integer)
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(ItmDesc2; 'IPOL  ' + "Sales Invoice Line".Description)
                        {
                        }
                        ...
                        column(ItmLineAmt; 0)// "Sales Invoice Line"."Line Amount")
                        {
                        }
                        ...
                        column(EntryTaxValue; EntryTaxValue)
                        {
                        }
                        ...
                        begin
                            ...
                            Check.InitTextVariable();
                            Check.FormatNoText(AmtinWordTotal, ROUND((AmtFinal + RoundOffAmnt), 1.0), '');//Total //16May2017
                            //Check.FormatNoText(AmtinWordTotal,ROUND((AmtFinal),0.01),'');//Total //19Apr2017
            ...
            begin
                ...
                //>>15Apr2017
                IF recCust15.GET("Sell-to Customer No.") THEN
                    SuppliersCode := recCust15."Our Account No.";

                //<<15Apr2017

                //Currency Code
                IF RecCust.GET("Sales Invoice Header"."Bill-to Customer No.") THEN;
                ...
                //RSPL-CAS-07249-Z7V2C3---Start
                CLEAR(V_freightTaxable);
                CLEAR(V_freightNonTaxable);
                /*  PostedStrOrdDetails.RESET;
                 PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Invoice No.", "Sales Invoice Header"."No.");
                 PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Structure Code", "Sales Invoice Header".Structure);
                 PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Tax/Charge Type", PostedStrOrdDetails."Tax/Charge Type"::Charges);
                 PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Tax/Charge Group", PostedStrOrdDetails."Tax/Charge Group", 'FREIGHT');
                 IF PostedStrOrdDetails.FINDFIRST THEN
                     REPEAT
                         PostedStrOrdDetails.CALCFIELDS(PostedStrOrdDetails."Structure Description");
                         IF PostedStrOrdDetails."Structure Description" = 'Freight (Taxable)' THEN
                             V_freightTaxable += PostedStrOrdDetails.Amount;
                         IF PostedStrOrdDetails."Structure Description" = 'Freight (Non-Taxable)' THEN
                             V_freightNonTaxable += PostedStrOrdDetails.Amount;
                     UNTIL PostedStrOrdDetails.NEXT = 0; */
                //RSPL-CAS-07249-Z7V2C3---End

                //>>11May2017
                CLEAR(FreightTax);
                CLEAR(FreightNonTax);

                IF V_freightTaxable <> 0 THEN
                    FreightTax := TRUE;

                IF V_freightNonTaxable <> 0 THEN
                    FreightNonTax := TRUE;

                //MESSAGE('Tax amount %1 \\ NonTaxAmt %2',V_freightTaxable,V_freightNonTaxable);
                ...
                IF recSIL.FINDFIRST THEN
                    IF recSIL."Item Category Code" <> 'CAT13' THEN BEGIN
                        ...
                        IF "Sales Invoice Header"."Posting Date" >= 20120104D THEN BEGIN //010412D
                            "Sales Invoice Line".RESET;
                            "Sales Invoice Line".SETRANGE("Sales Invoice Line".Type, "Sales Invoice Line".Type::Item);
                            ...
                            IF "Sales Invoice Line".FINDFIRST THEN
                                ...
                                BEGIN
                                    ...
                                    ItemVarParmResFinal.SETRANGE(ItemVarParmResFinal."Batch No./DC No", "Sales Invoice Line"."Lot No.");
                                    ItemVarParmResFinal.SETRANGE("Item No.", "Sales Invoice Line"."No.");
                                    IF ItemVarParmResFinal.FINDFIRST THEN
                                        REPEAT
                                            IF ItemVarParmResFinal."QC Test Report Generated" <> TRUE THEN BEGIN
                                                ERROR('Please Generate QC Test Report'); //<<robo-rahul
                                            END;
                                        UNTIL ItemVarParmResFinal.NEXT = 0;

                                    recItemLedger.RESET;
                                    recItemLedger.SETRANGE(recItemLedger."Item No.", "Sales Invoice Line"."No.");
                                    recItemLedger.SETRANGE(recItemLedger."Lot No.", "Sales Invoice Line"."Lot No.");
                                    recItemLedger.SETRANGE(recItemLedger."Location Code", "Sales Invoice Line"."Location Code");
                                    ...
                                    IF recItemLedger.FINDFIRST THEN BEGIN
                                        IF NOT (recItemLedger."Posting Date" = 20120331D) THEN BEGIN //310312D
                                            IF NOT ItemVarParmResFinal.FINDFIRST THEN BEGIN
                                                ERROR('QC Test Result Not Yet Posted for %1', ItemVarParmResFinal."Batch No./DC No");
                                            END;
                                        END;
                                    END;
                                END;
                ...
                */
                ...
                PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Invoice No.", "Sales Invoice Header"."No.");
                PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Tax/Charge Type", PostedStrOrdDetails."Tax/Charge Type"::Charges);
                PostedStrOrdDetails.SETRANGE(PostedStrOrdDetails."Tax/Charge Group", 'DISCOUNT');
                IF PostedStrOrdDetails.FINDFIRST THEN
                    REPEAT
                        DiscountCharge := DiscountCharge + PostedStrOrdDetails.Amount;
                    UNTIL PostedStrOrdDetails.NEXT = 0; */
                //<<27Apr2017
                ...
                /*  DetailedTaxEntry.RESET;
                 ...
                 DetailedTaxEntry.SETFILTER(DetailedTaxEntry."Tax Jurisdiction Code", '<>%1', "Tax Area Code");//11Apr2017
                 IF DetailedTaxEntry.FINDFIRST THEN BEGIN
                ...
                IF DetailedTaxEntry.FINDFIRST THEN
                    REPEAT
                        "AddlTax/Cess" += DetailedTaxEntry."Tax Amount";//11Apr2017
                                                                        //"AddlTax/Cess" := ROUND(("AddlTax/Cess" + DetailedTaxEntry."Tax Amount"));
                ...
                IF SIL.FINDSET THEN
                    REPEAT
                        ...
                        //AmtFinal := AmtFinal + SIL."Amount To Customer";//19Apr2017
                        ItmLineAmt1 := ItmLineAmt1 + SIL."Line Amount";
                        ItmTaxAmt1 := 0;// ItmTaxAmt1 + SIL."Tax Amount"; //19Apr2017
                        ItmInvDisc1 := ItmInvDisc1 + SIL."Inv. Discount Amount";//19Apr2017
                        ItmChrAmt1 := 0;// ItmChrAmt1 + SIL."Charges To Customer";//19Apr2017

                    UNTIL SIL.NEXT = 0;


                //>>19Apr2017 AmountFinal
                AmtFinal := ItmLineAmt1 + ItmExciseAmt1 + ItmeCessAmt1 + ItmeCessAmt1 + ItmTaxAmt1 - ItmInvDisc1 + ItmChrAmt1;
                //MESSAGE('AMount %1',AmtFinal);
                //<<19Apr2017 AmountFinal

                CLEAR(EntryTaxValue);
                CLEAR(EntryTaxDesc);
                /* recPostedStrOrdDetails.RESET;
                recPostedStrOrdDetails.SETRANGE("Invoice No.", "Sales Invoice Header"."No.");
                recPostedStrOrdDetails.SETRANGE("Tax/Charge Type", recPostedStrOrdDetails."Tax/Charge Type"::Charges);
                recPostedStrOrdDetails.SETRANGE("Tax/Charge Group", 'ENTRYTAX');
                IF recPostedStrOrdDetails.FINDSET THEN BEGIN
                    EntryTaxDesc := 'EntryTax @ ' + FORMAT(recPostedStrOrdDetails."Calculation Value") + ' ' + '%';
                    REPEAT
                        EntryTaxValue := EntryTaxValue + recPostedStrOrdDetails.Amount;
                    UNTIL recPostedStrOrdDetails.NEXT = 0;
                END; */

                //>>02 06Apr2017

                //Sales Invoice Header, Header (2) - OnPreSection()
                /* DetailedTaxEntry.RESET;
                DetailedTaxEntry.SETRANGE(DetailedTaxEntry."Document No.", "Sales Invoice Header"."No.");
                IF DetailedTaxEntry.FINDFIRST THEN;
                ...
                IF RecLocation2.FINDFIRST THEN BEGIN
                    IF (RecLocation2."State Code" = 'UP') AND ("Sales Invoice Header"."Posting Date" >= 20140111D) THEN //110114D
                        TaxInvoice := 'INVOICE';
                END;
    ...
    var
        ...
        "--11May2017": Integer;
        V_freightTaxable: Decimal;
        V_freightNonTaxable: Decimal;
        // recECC: Record 13708;
        recSIL: Record 113;
        ItemVarParmResFinal: Record 50015;
        recItemLedger: Record 32;
        FreightTax: Boolean;
        FreightNonTax: Boolean;
