report 50237 "RM Valuation Report --- 1 New"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/RMValuationReport1New.rdl';
    ...
    dataset
    {
        dataitem("Item Ledger Entry"; "Item Ledger Entry")
        {
            DataItemTableView = SORTING("Item Category Code", "Item No.", "Lot No.")
                                WHERE("Item Category Code" = FILTER('CAT01' | 'CAT05' | 'CAT06' | 'CAT04' | 'CAT07' | 'CAT08' | 'CAT09' | 'CAT10' | 'CAT16' | 'CAT15' | 'CAT19' | 'CAT21'),
                                      "Document Type" = FILTER("Transfer Shipment" | "Transfer Receipt" | "Purchase Receipt" | "Sales Return Receipt" | ' '));
            ...
            begin
                ...
                *///ss--
                ...
                IF PurcRcptLine.FIND('-') THEN BEGIN
                    ...
                    recPurchInvLine.SETRANGE(recPurchInvLine."Receipt No.", PurcRcptLine."Document No.");
                ...
                */
                ...
                IF ValueEntry.FINDSET THEN
                    REPEAT
                        Charges += ValueEntry."Cost per Unit";
                    UNTIL ValueEntry.NEXT = 0;
                //ss
                //>>06Sep2018
                /*ss
                ...
                *///ss
                ...
                ValueEntry.SETCURRENTKEY("Item Ledger Entry No.");
                ValueEntry.SETRANGE(ValueEntry."Item Ledger Entry No.", "Item Ledger Entry"."Entry No.");
                ValueEntry.SETFILTER(ValueEntry."Item Charge No.", '%1|%2|%3|%4|%5', 'AD 02', 'BO 02', 'CH 02', 'PKGMAT 01', 'PM 02');
                IF ValueEntry.FINDSET THEN
                    REPEAT
                ...
                IF ("Item Ledger Entry"."Entry Type" = "Item Ledger Entry"."Entry Type"::Purchase) AND
                    ...
                    Purchinvline.RESET;
                    Purchinvline.SETRANGE(Purchinvline."Receipt No.", "Item Ledger Entry"."Document No.");
                ...
                //BEGIN
                Purchinvline.RESET;
                Purchinvline.SETRANGE(Purchinvline."Receipt No.", GRNTRans);
                ...
                IF Purchinvline.FINDFIRST THEN BEGIN
                    ...
                    recPurchInvLine.RESET;
                    recPurchInvLine.SETRANGE(recPurchInvLine."Receipt No.", GRNTRans);
                    recPurchInvLine.SETRANGE(recPurchInvLine."No.", "Item Ledger Entry"."Item No.");
                    IF recPurchInvLine.FINDSET THEN
                        REPEAT
                            vLandedCostAmt += recPurchInvLine."Landed Cost";
                ...
                BEGIN
                  ...
                  Valueentryrec.SETFILTER(Valueentryrec."Item Charge No.",'');
    ...
    begin
        ...
        /*ssExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,FALSE,'@',1);//16
        ...
        ExcelBuf.AddColumn('',FALSE,'',TRUE,FALSE,FALSE,'@',1);//20*///ss
        ...
        *///ss
        ExcelBuf.AddColumn('Charges', FALSE, '', TRUE, FALSE, TRUE, '@', ExcelBuf."Cell Type"::Text);//21 ss
    ...
    begin
        ...
        *///ss
        ...
        //ssIF ("Item Ledger Entry"."Item Category Code" = 'CAT10') OR ("Item Ledger Entry"."Item Category Code" = 'CAT09') THEN
        //ssEcelBuf.AddColumn(ROUND((RateofItem),0.01),FALSE,'',FALSE,FALSE,FALSE,'#,####0.00',ExcelBuf."Cell Type"::Number)//15
        //ssELSE
        ExcelBuf.AddColumn(ROUND((ExpCost), 0.01), FALSE, '', FALSE, FALSE, FALSE, '#,####0.00', ExcelBuf."Cell Type"::Number);//15
        ExcelBuf.AddColumn(Charges, FALSE, '', FALSE, FALSE, FALSE, '#,####0.00', ExcelBuf."Cell Type"::Number);//16 ss
        /*ss
        ...
        *///ss
        ...
        //ssIF ("Item Ledger Entry"."Item Category Code" = 'CAT10') OR ("Item Ledger Entry"."Item Category Code" = 'CAT09') THEN
        //ssBEGIN
        //ssExcelBuf.AddColumn((TotalCharges + RateofItem),FALSE,'',FALSE,FALSE,FALSE,'#,####0.00',ExcelBuf."Cell Type"::Number);//22
        //ssExcelBuf.AddColumn((TotalCharges + RateofItem)*RemQty,FALSE,'',FALSE,FALSE,FALSE,'#,####0.00',ExcelBuf."Cell Type"::Number);//23
        //ssEND ELSE
        //ssBEGIN
        //ssExcelBuf.AddColumn(TotalCharges + ExpCost,FALSE,'',FALSE,FALSE,FALSE,'#,####0.00',ExcelBuf."Cell Type"::Number);//22
        //ssExcelBuf.AddColumn((TotalCharges + ExpCost)*RemQty,FALSE,'',FALSE,FALSE,FALSE,'#,####0.00',ExcelBuf."Cell Type"::Number);//23
        //ssEND;
        ExcelBuf.AddColumn(Charges + ExpCost, FALSE, '', FALSE, FALSE, FALSE, '#,####0.00', ExcelBuf."Cell Type"::Number);//22 ss
        ExcelBuf.AddColumn((Charges + ExpCost) * RemQty, FALSE, '', FALSE, FALSE, FALSE, '#,####0.00', ExcelBuf."Cell Type"::Number);//23 ss
