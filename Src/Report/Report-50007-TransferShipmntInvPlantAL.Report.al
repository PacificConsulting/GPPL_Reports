report 50007 "Transfer Shipmnt Inv_Plant_AL"
{
    DefaultLayout = RDLC;
    // RDLCLayout = './TransferShipmntInvPlantAL.rdlc';
    RDLCLayout = 'Src/Report Layout/TransferShipmntInvPlantAL.rdl';
    ...
    dataset
    {
        ...
        {
            column(Location; Loc2.Address + ' ' + Loc2."Address 2" + ' ' + Loc2.City + ' ' + Loc2."Post Code" + ' ' + State2 + ' Phone: ' + Loc2."Phone No." + ' ' + ' Email: ' + Loc2."E-Mail")
            {
            }
            column(InvNo; InvNo)
            {
            ...
            column(DateCST; Loc."W.e.f. Date(C.S.T No.)")
            {
            }
            ...
            dataitem(Integer; Integer)
            {
                ...
                dataitem(Integer1; Integer)
                {
                    ...
                    dataitem("Transfer Shipment Line"; "Transfer Shipment Line")
                    {
                        ...
                        column(ItmNo; "Transfer Shipment Line"."Item No.")
                        {
                        }
                        ...
                        column(ExciseProdPostingGroup_TransferShipmentLine; 0)// "Transfer Shipment Line"."Excise Prod. Posting Group")
                        {
                        }
                        ...
                        column(Ass_Value; 0)//"Transfer Shipment Line"."Assessable Value" / "Transfer Shipment Line"."Qty. per Unit of Measure")
                        {
                        }
                        column(TRF_price; "Transfer Shipment Line"."Unit Price" / "Transfer Shipment Line"."Qty. per Unit of Measure")
                        {
                        }
                        ...
                        begin
                            ...
                            */
                            ...
                            vCheck.InitTextVariable;
                            vCheck.FormatNoText(DescriptionLineAD, ROUND(AddlDuty, 0.01), '');
                            ...
                            IF recMRPmaster.FINDFIRST THEN BEGIN
                                MRPprice := recMRPmaster."MRP Price" * recMRPmaster."Qty. per Unit of Measure";
                            END;
            ...
            begin
                ...
                CLEAR(EcessAmt);
                CLEAR(AddlDuty);
                TransShipmentLine.RESET;
                TransShipmentLine.SETRANGE("Document No.", "No.");
                IF TransShipmentLine.FINDSET THEN
                    REPEAT
                        BEDAmount += 0;// TransShipmentLine."BED Amount";
                        EcessAmt += 0;//TransShipmentLine."eCess Amount";
                        SheCess += 0;// TransShipmentLine."SHE Cess Amount";
                        AddlDuty += 0;//TransShipmentLine."ADE Amount";
                        LineAmt += TransShipmentLine.Amount;
                    UNTIL TransShipmentLine.NEXT = 0;

                roundamt :=
                ROUND(LineAmt, 1) + ROUND(BEDAmount, 1) + ROUND(EcessAmt, 1)
                + ROUND(SheCess, 1) + ROUND(FrieghtAmount, 1) + ROUND(AdDutyAmount, 1);


                //<<

                //>>13Apr2017
                IF Loc2.GET("Transfer Shipment Header"."Transfer-from Code") THEN BEGIN
                    RecState.SETRANGE(RecState.Code, Loc2."State Code");
                    IF RecState.FINDFIRST THEN
                        State2 := RecState.Description;
                END;
                //>>13Apr2017
            end;
        }
