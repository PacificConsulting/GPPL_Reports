report 50230 "Bank Payment Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/BankPaymentReport.rdl';
    ...
    dataset
    {
        dataitem("Bank Account Ledger Entry"; "Bank Account Ledger Entry")
        {
            ...
            column(UserEmailId; UserEmailId)
            {
            }

            trigger OnAfterGetRecord()
            begin
                //Fahim 19-04-2023 for print Debit Account no.
                CLEAR(DebitAcNo);
                RecBankAc.RESET;
                RecBankAc.SETRANGE("No.", "Bank Account Ledger Entry"."Bank Account No.");
                IF RecBankAc.FINDFIRST THEN BEGIN
                    DebitAcNo := RecBankAc."Bank Account No.";
                END;
                // End

                //Fahim 19-04-2023 for print user email id
                CLEAR(UserEmailId);
                RecUserSertup.RESET;
                RecUserSertup.SETRANGE("User ID", "Bank Account Ledger Entry"."User ID");
                IF RecUserSertup.FINDFIRST THEN BEGIN
                    UserEmailId := RecUserSertup."E-Mail";
                END;
                //END
    ...
    var
        ...
        RecBankAc: Record 270;
        DebitAcNo: Text;
        RecUserSertup: Record 91;
        UserEmailId: Text;
}
