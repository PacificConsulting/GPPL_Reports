report 50076 "Invoice Industrial GST--LOGO"
{
    // 
    ...
    DefaultLayout = RDLC;
    RDLCLayout = 'Src/Report Layout/InvoiceIndustrialGSTLOGO.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(DataItem1000000077; Integer)
                {
                    DataItemTableView = SORTING(Number)
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            IF DetailGSTEntry.FINDSET THEN
                                REPEAT
                                    ...
                                    IF DetailGSTEntry."GST Component Code" = 'CGST' THEN BEGIN
                                        ...
                                        //MESSAGE('%1',DetailGSTEntry."GST Amount");
                            ...
                            */
                            ...
                            //ELSE
                            TotalAmt30 := /*"GST Base Amount"*/0 - (FREIGHTAmount + PKGFWDAmount);
                            //<<30Nov2017
                            ...
                            PCharge := 0;//"Charges To Customer"
                                         //ELSE
                            NCharge := 0;//"Charges To Customer";
                    ...
                    begin
                        ...
                        IF ("Sales Invoice Header"."Print Invoice" = TRUE) AND (USERID = 'GPUAE\FAHIM.AHMAD') THEN //18July2017
                        BEGIN
                            IF i = 1 THEN
                                InvType := 'ORIGINAL FOR RECIPIENT'
                            //InvType := 'ORIGINAL FOR BUYER'
                ...
                begin
                    ...
                    //IF NOT(USERID = 'GPUAE\UNNIKRISHNAN.VS') THEN
                    IF NOT (USERID = 'GPUAE\FAHIM.AHMAD') OR (USERID = 'GPUAE\RAVI.KHAMBAL') THEN BEGIN
    ...
    var
        ...
        HODCode: Text;
        RHeadCode: Text;
        SalespersonPurchaserHOD: Record 13;
        SalespersonPurchaserRHead: Record 13;
    ...
    var
        ...
        OutStr: OutStream;
        Instr: InStream;
        "Invoice Industrial GST--LOGO n": Report 50255;
    begin
        ...
        /*  ServerTempFile := FileMgmt.ServerTempFileName('pdf');
         IF REPORT.SaveAsPdf(50255, ServerTempFile, "Sales Invoice Header") THEN BEGIN */
        //Fahim
        ...
        if "Invoice Industrial GST--LOGO n".SaveAs('', ReportFormat::Pdf, OutStr) then begin
            ...
            //Fahim 03-01-2023
            //SMTPMail.AddCC (HODEmail+';fahim.ahmad@gpglobal.com;'+RHeadEmail);
            //SMTPMail.AddCC(RHeadEmail);//+';fahim.ahmad@gpglobal.com');
            //SMTPMail.AddAttachment(FileName,''); // Fahim 02-08-2022
            EmailMsg.AddRecipient(RecipientType::"Cc", RHeadEmail);
            //IF ServerTempFile <> '' THEN
            // SMTPMail.AddAttachment(ServerTempFile, 'Sales Invoice.pdf');
            // SMTPMail.Send;
    ...
    begin
        ...
        //Fahim 06-12-2022
        //HOD EMail ID
        HODCode := SalespersonPurchaser.HOD;
        CLEAR(SalespersonPurchaserHOD);
        SalespersonPurchaserHOD.RESET;
        SalespersonPurchaserHOD.SETRANGE(SalespersonPurchaserHOD.Code, HODCode);
        IF SalespersonPurchaserHOD.FINDFIRST THEN BEGIN
            HODEmail := SalespersonPurchaserHOD."E-Mail";
        END;
        //Regional Head Email ID
        RHeadCode := SalespersonPurchaser."Region Head Code";
        CLEAR(SalespersonPurchaserRHead);
        SalespersonPurchaserRHead.RESET;
        SalespersonPurchaserRHead.SETRANGE(SalespersonPurchaserRHead.Code, RHeadCode);
        IF SalespersonPurchaserRHead.FINDFIRST THEN BEGIN
            RHeadEmail := SalespersonPurchaserRHead."E-Mail";
        END;
        //End Fahim  06-12-2022
